...
public class RecommendationSaverData implements SaverData {
    ...
    Logger logger = new LoggerStandard("recommendation-saver-data");
    Connection con;
    ...
    public RecommendationSaverData() {
        try {
            con = DriverManager.getConnection(connectionStr, username, password);
            pstmt = con.prepareStatement(
                    "INSERT INTO recommendations (id, approval_id, route, active_moiety, dosing, interval) VALUES (?, ?, ?, ?, ?, ?)");
        } catch (Exception e) {
    ...
    public void saveData(Map<String, Object> data) {
        try {
            ...
            pstmt.setLong(2, Long.parseLong(data.get("approvalId").toString()));
            pstmt.setString(3, data.get("route").toString());
            pstmt.setString(4, data.get("activeMoiety").toString());
            pstmt.setString(5, data.get("dosing").toString());
            pstmt.setInt(6, Integer.parseInt(data.get("interval").toString()));
            pstmt.executeUpdate();
